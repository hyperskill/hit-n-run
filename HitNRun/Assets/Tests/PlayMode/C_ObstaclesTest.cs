...
public class C_ObstaclesTest
{
    ...
    public IEnumerator Check()
    {
        ...
        if (!PMHelper.CheckColorDifference(playerColor, obstacleColor))
        {
            Assert.Fail("The difference of colors between \"Player\" and obstacles should be visible!");
        }
        if (!PMHelper.CheckColorDifference(shotgunColor, obstacleColor))
        {
            Assert.Fail("The difference of colors between \"Shotgun\" and obstacles should be visible!");
        }
        if (!PMHelper.CheckColorDifference(backColor, obstacleColor))
        {
        ...
        foreach (GameObject g in GameObject.FindGameObjectsWithTag("Obstacle"))
        {
            ...
            if (!PMHelper.CheckObjectFits(gT, new Vector2(left, up), new Vector2(right, down)))
            {
