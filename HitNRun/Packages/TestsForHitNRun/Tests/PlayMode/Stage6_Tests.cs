...
public class Stage6_Tests
{
    ...
    public IEnumerator CheckSpeedAndColorIncrease()
    {
        ...
        float start = Time.unscaledTime;
        yield return new WaitUntil(() =>
            Vector2.Distance(firstEnemyT.position, playerT.position) < 0.25f ||
            (Time.unscaledTime - start) * Time.timeScale > 20);
        if (Vector2.Distance(firstEnemyT.position, playerT.position) >= 0.25f)
        {
        ...
        start = Time.unscaledTime;
        yield return new WaitUntil(() =>
            Vector2.Distance(secondEnemyT.position, playerT.position) < 0.25f ||
            (Time.unscaledTime - start) * Time.timeScale > 20);
        if (Vector2.Distance(secondEnemyT.position, playerT.position) >= 0.25f)
        {
            Assert.Fail("Enemies are not moving to a player, or moving too slow");
        }
